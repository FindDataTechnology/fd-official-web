...
Integrate the containerized `fd-open-data-mcp` with LibreChat by registering it as an mcpServer using the streamable-http transport, with in-cluster networking, bearer auth, configurable timeout, and a single shared Kubernetes Secret as the source of truth for the MCP token.
