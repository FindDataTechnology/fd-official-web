# mcp-container Specification

## Purpose

Build and run the `fd-open-data-mcp` as a containerized service on k3s: a reproducible Docker image with vendored dependencies, a Kubernetes Deployment with persistent SQLite storage, NodePort exposure with bearer auth, and push to the Harbor registry.
