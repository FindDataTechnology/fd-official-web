# k3s-install Specification

## Purpose

Provision a single-node k3s Kubernetes cluster on the Tencent host (`124.220.7.175`) that coexists with the existing nginx + static-site workload, using local-path storage and NodePort service exposure without an Ingress controller.
