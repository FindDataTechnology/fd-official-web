...
The live server is exposed for agent clients over HTTPS: add the remote MCP URL `https://www.finddatatech.cloud/mcp` with the org's bearer token. The domain swap has shipped — the web playground and remote MCP endpoint are both served over HTTPS.
